...
public class PropertiesLogger implements ApplicationListener<ApplicationEnvironmentPreparedEvent> {
    ...
    public void onApplicationEvent(@NonNull ApplicationEnvironmentPreparedEvent event) {
        ...
        log.debug("Logging "+propertySelectorType.forLogging());
        ...
        props.insert(0,
                "==============================================================================================="+
                    "                                Valeurs des properties pour :                                  "+
                        "%s" +
                    "==============================================================================================="
                .format(this.propertySourceNames.stream().reduce("",(l, e)->l+System.lineSeparator()+"- "+e )));
    ...
    private Optional<PropertySelectorEnum> getSelectorFromProperty(String property) {
        if(property!=null){
            try{
                ...
                log.trace("Impossible de convertir "+property+" en une constante de PropertySelectorEnum. Le PropertySelector par défaut sera utilisé.");
            }
        }
    ...
    private boolean filterFromPropertySelector(@NonNull String s) {
        if (! this.propertySelector.filter(s)){
            log.debug(s+ " ne commence pas par un des prefix retenus pour être loguée");
            return false;
        }
        return true;
    }
    ...
    private boolean sourceWillBeProcessed(PropertySource<?> propertySource) {
        ...
        if (ignoredPropertySources.contains(propertySource.getName())){
            log.debug(propertySource+ " sera ignorée");
            return false;
        }
        return true;
    }
    ...
    private boolean isEnumerable(PropertySource<?> propertySource) {
        if (! (propertySource instanceof EnumerablePropertySource)){
            log.debug(propertySource+ " n'est pas EnumerablePropertySource : impossible à lister");
            return false;
        }
        return true;
    }
